...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding bladeInputBinding = {};
    bladeInputBinding.binding = 0;
    bladeInputBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    bladeInputBinding.descriptorCount = 1;
    bladeInputBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    bladeInputBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding bladeOutputBinding = {};
    bladeOutputBinding.binding = 1;
    bladeOutputBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    bladeOutputBinding.descriptorCount = 1;
    bladeOutputBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    bladeOutputBinding.pImmutableSamplers = nullptr;
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings = { bladeInputBinding, bladeOutputBinding, numBladeBinding };
    // Create the descriptor set layout
    VkDescriptorSetLayoutCreateInfo layoutInfo = {};
    layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // debug { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2 },
...
void Renderer::CreateGrassDescriptorSets() {
    ...
    size_t n = scene->GetBlades().size();
    bladeDescriptorSets.resize(n);
    ...
    // loop over blades and prepare descriptor writes
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    // The descriptors point to Storage buffers which will hold the grass blades, the culled grass blades, and the output number of grass blades 
    size_t n = scene->GetBlades().size();
